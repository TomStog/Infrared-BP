...
if __name__ == "__main__":
  ...
  data = pd.read_csv('/content/blood_pressure_mean_std.csv')
  ...
  for i in range(7,8):
    ...
    for _ in range(10):
      ...
      gam_D_BP = LinearGAM(n_splines=i).gridsearch(X, y_D_BP, lam=lams, progress=False)
      gam_S_BP = LinearGAM(n_splines=i).gridsearch(X, y_S_BP, lam=lams, progress=False)
